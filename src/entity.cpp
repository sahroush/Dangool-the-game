...
void Entity::set_origin(float x, float y) { sprite.setOrigin(x, y); }
...
void Entity::set_rotation(float deg) { sprite.setRotation(deg); }
...
bool Entity::check_top_collision(FloatRect me, FloatRect block) {
  ...
  return me.top < block.top + block.height and
         overlap.width >= overlap.height and me.top > block.top;
}

bool Entity::check_bottom_collision(FloatRect me, FloatRect block) {
  auto overlap = find_overlap_rect(me, block);
  return me.top + me.height > block.top and overlap.width >= overlap.height and
         me.top + me.height < block.top + block.height;
}

bool Entity::check_left_collision(FloatRect me, FloatRect block) {
  auto overlap = find_overlap_rect(me, block);
  return me.left > block.left and me.left < block.left + block.width and
         overlap.height >= overlap.width;
}

bool Entity::check_right_collision(FloatRect me, FloatRect block) {
  auto overlap = find_overlap_rect(me, block);
  return me.left + me.width > block.left and
         me.left + me.width < block.left + block.width and
         overlap.height >= overlap.width;
}
...
Vector2f Entity::get_position() { return sprite.getPosition(); }
...
void Entity::set_position(double x, double y) { sprite.setPosition(x, y); }
...
void Entity::rotate90() {
  if (!moved) {
    moved = true;
    sprite.move(sprite.getGlobalBounds().width / 2.f,
...
FloatRect Entity::get_rect() { return sprite.getGlobalBounds(); }

double Entity::get_width() { return sprite.getGlobalBounds().width; }

double Entity::get_height() { return sprite.getGlobalBounds().height; }
