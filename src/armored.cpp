...
int ArmoredEnemy::calc_frame(){
    ...
    switch(state){
        case(armored_enemy::WALKING):
            return walk_frames[(update_count/armored_enemy::ANIMATION_UPDATE_STEP)%walk_frames.size()];
            break;
        case(armored_enemy::SLEEPING):
            return sleep_frames[(update_count/armored_enemy::ANIMATION_UPDATE_STEP)%sleep_frames.size()];
            break;
        case(armored_enemy::GOING_TO_SLEEP):
            return going_to_sleep_frames[(update_count/armored_enemy::ANIMATION_UPDATE_STEP)%going_to_sleep_frames.size()];
            break;
        case(armored_enemy::WAKING_UP):
            return waking_up_frames[(update_count/armored_enemy::ANIMATION_UPDATE_STEP)%waking_up_frames.size()];
            break;
    }
}
...
void ArmoredEnemy::update_avatar(){
    ...
    switch(state){
        ...
        case(armored_enemy::SLEEPING):
            ...
            draw_external_sprite(external_frames[cur_frame].left, external_frames[cur_frame].top,
