...
bool NormalEnemy::is_hittable() { return state != normal_enemy::KNOCKED; }

bool NormalEnemy::is_lethal() { return state != normal_enemy::KNOCKED; }
...
int NormalEnemy::calc_frame() {
  ...
  case (normal_enemy::KNOCKED):
    return panic_frames[(update_count / normal_enemy::ANIMATION_UPDATE_STEP) %
                        panic_frames.size()];
    break;
  case (normal_enemy::STAGE1):
    return tank_frames[(update_count / normal_enemy::ANIMATION_UPDATE_STEP) %
                       tank_frames.size()];
    break;
  case (normal_enemy::STAGE2):
    return walk_frames[(update_count / normal_enemy::ANIMATION_UPDATE_STEP) %
                       walk_frames.size()];
    break;
  }
}
