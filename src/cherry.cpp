...
bool Cherry::is_hittable() { return state == cherry::IDLE; }

bool Cherry::is_lethal() { return 0; }

bool Cherry::is_killable() { return state == cherry::MOVING; }
...
int Cherry::calc_frame() {
  ...
  switch (state) {
  case (cherry::IDLE):
    return idle_frames[(update_count / cherry::ANIMATION_UPDATE_STEP) %
                       idle_frames.size()];
    break;
  case (cherry::KNOCKED):
    return panic_frames[(update_count / cherry::ANIMATION_UPDATE_STEP) %
                        panic_frames.size()];
    break;
  case (cherry::MOVING):
    return move_frames[(update_count / cherry::ANIMATION_UPDATE_STEP) %
                       move_frames.size()];
    break;
  }
}
...
void Cherry::fall() { ay = cherry::GRAVITY; }
...
void Cherry::be_gone() { display = false; }
