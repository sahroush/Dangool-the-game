...
MainMenu::MainMenu() {
  ...
  bg.setTexture(bgs[0]);
  int tot_width =
      start_button.get_width() + BUTTON_MARGIN + exit_button.get_width();
  start_button.set_position({(WIDTH - tot_width) / 2.f, HEIGHT / 2.f});
  exit_button.set_position(
      {(WIDTH - tot_width) / 2.f + BUTTON_MARGIN + start_button.get_width(),
       HEIGHT / 2.f});
...
bool MainMenu::check_start() { return start_button.get_status(); }

bool MainMenu::check_exit() { return exit_button.get_status(); }
