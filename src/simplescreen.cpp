...
SimpleScreen::SimpleScreen(string message, string music_name, string font_name,
                           int text_size) {
...
void SimpleScreen::render(RenderWindow &window) {
  ...
  text.setPosition((WIDTH - text.getGlobalBounds().width) / 2.f,
                   (HEIGHT - text.getGlobalBounds().height) / 2.f);
  window.draw(text);
}

void SimpleScreen::stop() { music.stop(); }